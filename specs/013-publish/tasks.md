...
- [X] T013-S2 Replace only the `POST /v1/fee-quote` and `POST /v1/entries`
  `501` stubs with real handlers backed by the publish domain.
- [X] T013-S2 Make operator fee schedule and TTL horizon configurable and
  surface those values in `GET /v1/operator`.
- [X] T013-S2 Unit-test the HTTP JSON/status behavior for fee quote, publish
  success, and mapped publish failures.
- [X] T013-S2 Run focused server tests and `./gate.sh`.
