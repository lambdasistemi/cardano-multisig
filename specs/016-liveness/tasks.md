...
- [X] T016-S2 Extend `GET /v1/entries/{id}` to include
  `liveness.inputs_unspent` and `liveness.phase1_ok`, while rendering `expired`
  status when persisted.
- [X] T016-S2 Start the supervised monitor beside Warp in `runServer`, sharing
  the store and N2C provider and retrying after transient monitor failures.
- [X] T016-S2 Add focused HTTP/runtime tests for the liveness payload and
  monitor startup shape without requiring a live node.
- [X] T016-S2 Run focused server tests and `./gate.sh`, then commit the slice.
